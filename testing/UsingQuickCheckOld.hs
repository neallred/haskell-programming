...
runAllListOrderedQc =
  quickCheck prop_ListOrderedInt >> quickCheck prop_ListOrderedBool >>
...
runQcPlus :: IO ()
runQcPlus = quickCheck prop_PlusAssociative >> quickCheck prop_PlusCommutative
...
runQcMult :: IO ()
runQcMult = quickCheck prop_MultAssociative >> quickCheck prop_MultCommutative

-- 5.
myQuotRem x y = (quot x y) * y + (rem x y) == x

prop_QuotRem :: Int -> Int -> Property
prop_QuotRem x y = x > 0 && y > 0 ==> myQuotRem x y
...
prop_PrefixSuffix xs n =
  isPrefixOf prefix xs && isSuffixOf (reverse prefix) (reverse xs)
  where
    prefix = take n xs
...
prop_PrimeSum_v1 p q = p > 2 && q > 2 && isPrime p && isPrime q ==> even (p + q)

prop_PrimeSum_v1' :: Int -> Int -> Property
prop_PrimeSum_v1' p q =
  p > 2 && q > 2 && isPrime p && isPrime q ==>
  classify (p < 20 && q < 20) "trivial" $
  even (p + q)
